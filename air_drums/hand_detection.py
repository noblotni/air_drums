...
def detect_skin(frame: np.ndarray):
    ...
    # Blur image
    blur_frame = cv.GaussianBlur(frame, GAUSSIAN_KERNEL_SIZE, GAUSSIAN_SIGMA_X)
...
def find_hands_contours(frame: np.ndarray, skin_mask: np.ndarray):
    ...
    for contour in contours:
        ...
        if cnt_area > MIN_CNT_AREA:
            ...
            cv.circle(frame, center=center, radius=0, color=(255, 0, 0), thickness=4)
            distance = np.linalg.norm(box[0, 0] - center)
    return frame
